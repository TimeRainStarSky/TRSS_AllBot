...
docker run -itd -h TRSS-AllBot --name $DKNAME -v "$DIR":/root/TRSS_AllBot -v /var/run/docker.sock:/var/run/docker.sock --restart always $([ $DKNAME = TRSS_AllBot ]&&echo "-p 2536:2536 -p 8080-8081:8080-8081 -p 8088:8088 -p 13579:13579 -p 50831:50831 -p 54980:54980"||echo "-P") trss:allbot||abort "Docker 容器启动失败"
mkdir -vp "$CMDPATH"&&
