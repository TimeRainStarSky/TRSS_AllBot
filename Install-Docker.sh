...
docker run -itd -h TRSS-AllBot --name $DKNAME -v "$DIR":/root/TRSS_AllBot $([ -S /var/run/docker.sock ]&&echo "-v /var/run/docker.sock:/var/run/docker.sock") --restart always $([ $DKNAME = TRSS_AllBot ]&&echo "--network host"||echo "-P") trss:allbot||abort "Docker 容器启动失败"
mkdir -vp "$CMDPATH"&&
